...
def test_main_ldap_connection_error_empty_bind_password_keeps_message(monkeypatch):
    """main(): empty LDAP bind password must not mangle the LDAP error."""
    mod = _load()
    monkeypatch.setattr(mod, "oracledb_exists", True, raising=False)
    monkeypatch.setattr(mod, "ldap_module_exists", True, raising=False)
    ...
    monkeypatch.setattr(mod, "ldap", FailingLdap, raising=False)
    Mod = _make_ldap_mod(_ldap_params(ldap_bindpassword=""))
    monkeypatch.setattr(mod, "AnsibleModule", Mod)
